...
class Db_operations:
    ...
    def connect_db(self):
        try:
            connection = pymysql.Connect(host='localhost', port=3306, user='root', password='Bilal1234#', database='bilal_db', charset='utf8')
            print('DB connected')
            return connection
        except:
    ...
    def create_db(self):
        ...
        query = 'create database IF NOT EXISTS bilal_db;'
    ...
    def insert_row(self, person):
        try:
            ...
            print("📦 Executing INSERT with:", person_tuple)
            ...
            print("✅ New row inserted with ID:", id)
            return id
    ...
    def list_all_rows(self):
        ...
        try:
            ...
            cursor = connection.cursor()
            count = cursor.execute(query)
            rows = []  # 🔁 Always define rows as a list
            if count == 0:
            ...
            cursor.close()
            self.disconnect_db(connection)
            return rows  # ✅ Always returns a list
        except Exception as e:
            print('Error in reading rows:', e)
    ...
    def get_latest_row_id(self):
        ...
        if connection is None:
            print("Failed to connect to DB. Cannot fetch latest row id.")
            return None
