...
from pymysql_sic1 import Person, Db_operations
...
def persons_create():
    try:
        ...
        print("📥 JSON received from Thunder Client:", body)

        new_person = Person(body['name'], body['gender'], body['dob'], body['location'])
        print("✅ Creating Person object:", new_person.name, new_person.gender, new_person.dob, new_person.location)

        id = persons.insert_row(new_person)
        print("✅ Inserted into DB, got ID:", id)

        person = persons.search_row(id)
        print("🔍 Fetched inserted person:", person)
